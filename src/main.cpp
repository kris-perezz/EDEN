...
int main() {
  ...
  glBufferData(GL_ARRAY_BUFFER, objVertices.size() * sizeof(float),
               objVertices.data(), GL_STATIC_DRAW);

  // Bind and fill EBO with OBJ index data
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               objIndices.size() * sizeof(unsigned int), objIndices.data(),
               GL_STATIC_DRAW);
  ...
  // --- Camera Setup ---
  Camera camera;
  // Configure perspective projection: 45° FOV, correct aspect, near and far
  // clipping planes
  camera.setPerspectiveProjection(glm::radians(45.0f), 640.0f / 480.0f, 0.1f,
                                  100.0f);
  // Set view: camera positioned at (0,0,3) looking at the origin
  camera.setViewTarget(glm::vec3(0.0f, 0.0f, 3.0f),
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Update camera uniforms each frame (if the camera is moving, update
    // accordingly)
    ...
    glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(objIndices.size()),
                   GL_UNSIGNED_INT, 0);
    // Rendering
